...
const charsInfo = font.chars.reduce((r, v) => {
    r[v.char] = v;
    return r;
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const drawChars = gl.command(PrimitivesType.TriangleStrip, {
        uniforms: {
        ...
        vertex({ aPosition, iXY, iUV, iSize, uTransform, uTextureSize }) {
            ...
            return {
                gl_Position: uTransform.mul(iXY.add(position).cat(1)).take(0, 1).cat(val(0, 1)),
                vUV: val(0, 1).sub(iUV.add(position).div(uTextureSize).mul(val(-1, 1))),
            };
        },
        fragment({ uTexture, vUV, uAlpha }) {
            ...
            return {
                gl_FragColor: val(1, 1, 1, median.smoothstep(0.43, 0.57).mul(uAlpha).pow(2.2)),
            };
        }
    }).setUniforms({
        uTexture: texture,
